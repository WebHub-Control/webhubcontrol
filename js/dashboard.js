document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('statTotalDev').textContent = STORE.formatBRL(totals.totalDev);
  document.getElementById('statTotalAgencia').textContent = STORE.formatBRL(totals.totalAgencia);
  document.getElementById('statTotalEu').textContent = STORE.formatBRL(totals.totalEu);
  ...
  document.getElementById('statCardDev').addEventListener('click', () => {
    ...
    clients.forEach(c => {
      ...
      const devValue = STORE.splitValues(c).dev;
      if (!byDev[name]) byDev[name] = { total: 0, clients: [] };
      byDev[name].total += devValue;
      byDev[name].clients.push({ c, devValue });
    });
    const groupsHTML = Object.keys(byDev)
      .sort((a, b) => byDev[b].total - byDev[a].total)
      .map(name => {
        const g = byDev[name];
        const rows = g.clients.map(({ c, devValue }) => `
          <div class="modal-row">
            <div class="modal-row-info"><strong>${c.empresa || 'Sem nome'}</strong><span>${c.devPago ? 'Pago' : 'A pagar'}</span></div>
            <div class="modal-row-value">${STORE.formatBRL(devValue)}</div>
          </div>`).join('');
        return `<div class="modal-group"><div class="modal-group-title">${name} — ${STORE.formatBRL(g.total)}</div>${rows}</div>`;
      }).join('');
    openModal('Repassado para devs', groupsHTML + `<div class="modal-total-row"><span>Total</span><span>${STORE.formatBRL(totals.totalDev)}</span></div>`);
  });
  ...
  document.getElementById('statCardAgencia').addEventListener('click', () => {
    ...
    const rows = clients.map(c => {
      const agenciaValue = STORE.splitValues(c).agencia;
      return `
        <div class="modal-row">
          <div class="modal-row-info"><strong>${c.empresa || 'Sem nome'}</strong><span>${c.splitAgencia}% de ${STORE.formatBRL(c.valor)}</span></div>
          <div class="modal-row-value">${STORE.formatBRL(agenciaValue)}</div>
        </div>`;
    }).join('');
    openModal('Repassado para a agência', `
      <div class="modal-group">${rows}</div>
      <div class="modal-total-row"><span>Total</span><span>${STORE.formatBRL(totals.totalAgencia)}</span></div>`);
  });
});
