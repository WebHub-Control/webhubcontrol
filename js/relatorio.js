document.addEventListener('DOMContentLoaded', () => {
  ...
  function computeSummary(clients) {
    /* Usa os valores reais (recebido/repassado), os mesmos do dashboard — não o
       combinado "no papel" — pra o relatório não contradizer o painel principal. */
    const acc = { totalFechado: 0, totalRecebido: 0, totalDev: 0, totalAgencia: 0, totalEu: 0, count: clients.length };
    clients.forEach(c => {
  ...
  function render() {
    ...
    statsEl.innerHTML = `
      ...
      <div class="stat-card">
        <div class="stat-icon"><svg class="icon"><use href="#i-inbox"/></svg></div>
        <div>
  ...
  document.getElementById('downloadPdfBtn').addEventListener('click', () => {
    ...
    const summaryRows = [
      ['Clientes fechados', String(s.count)],
    ...
    if (!clients.length) {
      ...
      doc.setFontSize(10.5);
      doc.setTextColor(...GRAY);
      doc.text('Nenhum cliente fechado nesse período.', marginX, y);
    } else {
      const cols = [
        ...
        { label: 'Valor', width: contentWidth * 0.15, get: c => STORE.formatBRL(c.valor) },
      ...
      clients.forEach((c, i) => {
        ...
        const cellLines = cols.map(col => doc.splitTextToSize(String(col.get(c)), col.width - 5));
